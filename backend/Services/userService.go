...
func Signin(username string, password string) *Models.User {
	var user = new(Models.User)
	//password = "123456"
	//Database.Db.Where("username = ? AND password = ?", username, password).Preload("user").First(&user)
	Database.Db.Where("username = ? AND password = ?", username, password).Preload("user").Find(&user)
